...
public class MatrixUtil {
    ...
    /**
     * Matrix add lab3.matrix
     * @param m1    the first lab3.matrix
     * @param m2    the second lab3.matrix
    ...
    /**
     * Multiply lab3.matrix and number
     * @param m     lab3.matrix
     * @param c     number
     * @return      result of their product
     */
    public static double[][] multiply(final double[][] m, final double c) {
        ...
        IntStream.range(0, m.length).forEach(i -> IntStream.range(0, m.length).forEach(j -> ans[i][j] = m[i][j] * c));
        return ans;
    }

    /**
     * Multiply lab3.matrix and vector
     * @param m     lab3.matrix
     * @param v     vector
     * @return      result of their product
     */
    ...
    /**
     * Generate diagonal lab3.matrix with define size and condition number
     * @param n     lab3.matrix's size
     * @param k     condition number
     * @return      diagonal lab3.matrix with parameters n and k
     */
     ...
     */
    public static double[] generateUnaryVector(int n) {
        double[] v = new double[n];
        Arrays.fill(v, 1.0);
        return v;
    }
    ...
    public static double[][] transpose(double[][] m) {
        double[][] transposeM = new double[m.length][m.length];
    ...
    /**
     * Print point in special format
     * @param x     point
     */
    public static void print(final double[] x) {
        System.out.printf(Locale.US, "%.15e, %.15e\n", x[0], x[1]);
    }

    /**
     * Print vector in special format
     * @param x1    begin
     * @param x2    end
     */
    public static void print(final double[] x1, final double[] x2) {
        System.out.printf(Locale.US, "vector(%s,%s)\n", pointToString(x1), pointToString(x2));
    }

    /**
     * Convert to string point coordinates
     * @param x     point
     * @return      string with point
     */
    private static String pointToString(final double[] x) {
